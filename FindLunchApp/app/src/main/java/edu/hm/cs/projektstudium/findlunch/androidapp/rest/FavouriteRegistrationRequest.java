...
/**
 * The type Favourite Registration request
 * allows to register a favourite restaurant
 * for a certain user, through
 * the FindLunch REST API.
 */
public class FavouriteRegistrationRequest extends FavouriteRequest {
    /**
    ...
    public FavouriteRegistrationRequest(OnHttpRequestFinishedCallback context,
        ...
        requestUrl = "http://" +  requestHost + ":" + requestPort + "/api/register_favorite/{restaurant_id}";
    }
    ...
    public void performRequest() {
        if (Connectivity.isConnected((Context)context)) {
            ...
            try {
                // Make the HTTP PUT request to the Basic Auth protected URL
                ResponseEntity<FavouriteRegistrationStatus> responseEntity = restTemplate.exchange(requestUrl,
                        HttpMethod.PUT, requestEntity, FavouriteRegistrationStatus.class, parameters);
