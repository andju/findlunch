...
/**
 * The Class RestaurantTypeRestControllerIT.
 */
...
@IntegrationTest()
public class RestaurantTypeRestControllerIT {

	@Autowired
	private RestaurantTypeRepository restaurantTypeRepo;

	/** The Constant TYPE_API. */
	private static final String TYPE_API = "/api/restaurant_types";
	...
	/**
	 * Test restaurant type rest controller.
	 */
	@Test
	public void testRestaurantTypeRestController() {
		List<RestaurantType> expectedResult = restaurantTypeRepo.findAll();

		Response response = RestAssured.given().when().get(TYPE_API).then().statusCode(200).contentType(ContentType.JSON)
				.body("list.size()", Matchers.is(expectedResult.size())).extract().response();

		DayOfWeek[] actualResult = response.as(DayOfWeek[].class);

		// Check if correct results are present (Only fields that are sent via
		// the rest interface are checked)
